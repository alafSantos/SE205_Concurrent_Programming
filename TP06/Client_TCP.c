...
int main(int argc, char *argv[])
{
  ...
  if ((Sock_Des = socket(PF_INET, SOCK_STREAM, 0)) < 0) // 1.2.2
  {
  ...
  if ((ph = gethostbyname(argv[1])) == (struct hostent *)0) // 1.2.2
  {
...
#define PORT_SERVEUR 45678 // 1.2.2

  Le_Serveur.sin_port = htons(PORT_SERVEUR); // 1.2.2

  /* connexion vers le serveur */
  if (connect(Sock_Des, (struct sockaddr *)&Le_Serveur, sizeof(Le_Serveur)) < 0) // 1.2.2
  { 
  ...
  while (fgets(Buf, sizeof(Buf), stdin))
  {
    if (!strncmp(Buf, "fin", 3))
      break;
    if (write(Sock_Des, Buf, strlen(Buf)) < 0) // 1.2.2
      perror("Erreur d'ecriture vers le serveur");
  }
