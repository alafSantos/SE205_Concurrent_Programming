...
int main(void)
{
  ...
  if ((Sock_Serv = socket(PF_INET, SOCK_STREAM, 0)) < 0) // 1.2.1
  {
  ...
  **********************************************************************/
  ...
  Le_Serveur.sin_port = htons(PORT_SERVEUR); // 1.2.1
  ...
  */
  ...
  if (bind(Sock_Serv, (struct sockaddr *)&Le_Serveur.sin_addr.s_addr, sizeof(struct sockaddr_in)) < 0) // 1.2.1
  {
  ...
  while (1)
  {
    ...
    if ((Sock_Com = accept(Sock_Serv, (struct sockaddr *)0, (int *)0)) == -1) // 1.2.1
    {
...
void Gerant_Comm()
{
  ...
  do
  {
    ...
    if ((Ret_Read = read(Sock_Com, Buf, sizeof(Buf))) < 0) // 1.2.1
    {
